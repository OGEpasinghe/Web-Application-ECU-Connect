...
def loginPage(request):
     ...
     #get the email and pass from the post request
     if request.method == 'POST':
          
          email=request.POST.get('email').lower()
          password=request.POST.get('password')

        #check for relevant user from the database
          try:
            user = User.objects.get(email=email)
          except:
            messages.error(request, 'User does not exist')  #if not available

          user = authenticate(request, email=email, password=password)

          if user is not None: #if matching user is found it logs you in
            login(request, user)
            return redirect('home')
          else:
            messages.error(request, 'Username or password does not exist') #authentication failure
     context = {'page': page}
...
#checks if form is valid, if all data valid
#saves the data, logs the user in, and directs them to home.
...
def thread(request, pk):
     ...
     thread = get_object_or_404(Thread, id=pk) #get the thread according to the given PK, if not, error.
     thread_comments = thread.comment_set.all() #get all the comments related to the thread
     participants = thread.participants.all() #get all the participants related to the thread

    #if a user comments, add the comment, then add the user to the participants list.
     if request.method == 'POST':
...
def createThread(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            thread = form.save(commit=False)
            thread.host = request.user #host is set to the current authenticated user
